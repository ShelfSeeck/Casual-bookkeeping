...
- 客户端先在 IndexedDB 事务外完成网络下载与 JSON 解析，再开一个 Dexie 事务一次完成：应用本页所有业务变化（写 `after_json` + `after_version` 到业务表）、写入本地 operations 镜像、更新 `applied_server_seq` 为本页最后一条 `server_seq`。
