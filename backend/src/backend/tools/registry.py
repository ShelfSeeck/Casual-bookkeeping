...
def register_tool(func: Any = None, *, requires_approval: bool = False) -> Any:
    ...
    def decorator(inner: Any) -> Any:
        ...
        _TOOL_REGISTRY[tool_name] = (inner, requires_approval)
        return inner
...
def build_tools(allowed: list[str] | None = None) -> list[Tool[Any]]:
    ...
    for tool_name, (func, requires_approval) in _TOOL_REGISTRY.items():
        if allowed_set is not None and tool_name not in allowed_set:
            continue
        tools.append(Tool(func, requires_approval=requires_approval))
    return tools
