...
// 表单状态
const selectedCustomerId = ref<number>(1) // 默认 001-张老板
const selectedCategoryName = ref('洗水')
const selectedSubcategoryName = ref('酵洗')
const unit = ref('件')
const quantityStr = ref('3000')
const unitPriceStr = ref('1.50')
const orderDate = ref('2026-08-15')
...
const currentCustomer = computed(() => {
  return prototypeState.customers.find((c) => c.customerId === selectedCustomerId.value) || prototypeState.customers[0]
})

// 过滤后的客户列表
const filteredCustomers = computed(() => {
  if (!customerSearch.value.trim()) {
    return prototypeState.customers
  }
  const q = customerSearch.value.trim().toLowerCase()
  return prototypeState.customers.filter((c) => {
...
const activeCategory = computed(() => {
  return prototypeState.categories.find((c) => c.name === selectedCategoryName.value)
})
...
<template>
  <div class="cb-console-wrapper">
    ...
    <div class="cb-console-panel">
      ...
      >
        <span>今日已录流水 ({{ prototypeState.todayOrders.value.length }} 笔)</span>
    ...
    <div id="desk-today-flow-block" class="cb-flow-stream-panel">
      <div class="cb-stream-header">
        ...
        <span class="cb-stream-count-badge">{{ prototypeState.todayOrders.value.length }} 笔</span>
      </div>

      <div v-if="prototypeState.todayOrders.value.length === 0" class="cb-stream-empty">
        今日暂无已录单据
      </div>

      <div v-else class="cb-stream-items-wrap">
        <button
          v-for="item in prototypeState.todayOrders.value"
          :key="item.orderId"
          type="button"
          class="cb-stream-row-btn cb-pressable"
          @click="prototypeState.setTab('ledger')"
        >
