...
import type { MockCategory } from '../../mock/mockData'
...
const filteredCustomerList = computed(() => {
  ...
  if (!kw) return prototypeState.customers
  return prototypeState.customers.filter(
    (c) =>
...
const formValidFrom = ref('2026-08-15')

function resetCustomerForm() {
  formCode.value = ''
  formDisplayName.value = ''
  formCustomerName.value = ''
  formValidFrom.value = '2026-08-15'
}
...
function submitNewCategory() {
  ...
  if (prototypeState.categories.some((c) => c.name === catName)) {
    alert('该大类名称已存在，请勿重复创建')
    return
  }

  prototypeState.categories.push({
    categoryId: Date.now(),
    name: catName,
    isActive: true,
    subcategories: [],
  ...
  alert(`服务大类「${catName}」创建成功！现在可以为其添加具体的洗涤项目。`)
}

function deleteCategory(catId: number, catName: string) {
  if (confirm(`确定要删除大类「${catName}」及其下所有项目吗？`)) {
    const idx = prototypeState.categories.findIndex((c) => c.categoryId === catId)
    if (idx !== -1) {
      prototypeState.categories.splice(idx, 1)
    }
  }
}

// 在已有大类下添加具体小类项目（小类项目拥有名称与默认计价单位）
const activeAddingCatId = ref<number | null>(null)
const inlineSubName = ref('')
const inlineSubUnit = ref('件')

function startAddSubcategory(catId: number) {
  activeAddingCatId.value = catId
  inlineSubName.value = ''
...
function saveInlineSubcategory(cat: MockCategory) {
...
function deleteSubcategory(cat: MockCategory, subIdx: number, subName: string) {
  if (confirm(`确定要删除项目「${subName}」吗？`)) {
          ...
          <div class="md3-card md3-card--outlined md3-list-container">
            ...
            >
              ...
              <div class="md3-list-item-trailing">
                <span class="md3-list-item-meta cb-tabular-nums">{{ prototypeState.customers.length }} 家客户</span>
            ...
            >
              ...
              <div class="md3-list-item-trailing">
                <span class="md3-list-item-meta cb-tabular-nums">{{ prototypeState.categories.length }} 个大类</span>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M9 18l6-6-6-6"/>
                </svg>
              </div>
        ...
        <section class="md3-list-group" aria-label="数据同步">
          ...
          <div class="md3-card md3-card--outlined md3-list-container">
            ...
            >
              ...
              <div class="md3-list-item-trailing">
                <span class="md3-badge-success">已同步</span>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M9 18l6-6-6-6"/>
                </svg>
              </div>
        ...
        <section class="md3-list-group" aria-label="账号与设备">
          ...
          <div class="md3-card md3-card--outlined md3-list-container">
            <div class="md3-list-item">
              ...
              <div class="md3-list-item-trailing">
                <span class="md3-mono-text cb-tabular-nums">13800138000</span>
              </div>
            </div>
            ...
            <div class="md3-list-item">
              ...
              <div class="md3-list-item-trailing">
                <span class="md3-mono-text">dev_a1b2c3d4</span>
              </div>
            </div>
          </div>
        </section>
          ...
          >
            <div class="cb-cat-header-row">
              ...
              <button
                ...
                @click="deleteCategory(cat.categoryId, cat.name)"
              >
                删除大类
              </button>
            </div>
            ...
            <div class="md3-chip-set">
              ...
              >
                ...
                <button
                  ...
                  @click.stop="deleteSubcategory(cat, subIdx, sub.name)"
                >
                  ✕
                </button>
              </div>
            ...
            <div v-if="activeAddingCatId === cat.categoryId" class="md3-inline-add-container">
            ...
            <button
              ...
              @click="startAddSubcategory(cat.categoryId)"
            >
              + 添加服务项目与默认单位
            </button>
          </div>

          <div v-if="prototypeState.categories.length === 0" class="cb-empty-state">
    ...
    <div v-else-if="currentSubPage === 'sync'" class="cb-page-container">
      ...
      <main class="cb-subpage-body">
        <div class="md3-card md3-card--elevated cb-sync-status-hero">
          ...
          <h2 class="cb-sync-hero-title">数据已完全同步</h2>
          <p class="cb-sync-hero-desc">所有工单与基础档案均已与云端对齐</p>
        </div>
        ...
        <div class="md3-card md3-card--outlined md3-list-container">
          <div class="md3-list-item">
            ...
            <div class="md3-list-item-trailing">
              <span class="md3-mono-text">0 笔</span>
            </div>
          </div>
          ...
          <div class="md3-list-item">
            <div class="md3-list-item-content">
              <span class="md3-list-item-headline">上次同步时间</span>
            </div>
            <div class="md3-list-item-trailing">
              <span class="md3-mono-text cb-tabular-nums">刚刚 (15:40)</span>
            </div>
          </div>
        </div>
