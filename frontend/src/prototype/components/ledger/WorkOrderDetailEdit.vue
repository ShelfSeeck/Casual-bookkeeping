...
const currentCustomer = computed(() => {
  return (
    prototypeState.customers.find((c) => c.customerId === selectedCustomerId.value) ||
    prototypeState.customers[0]
  )
})
...
function handleSave() {
  ...
  prototypeState.updateWorkOrder(props.order.orderId, {
...
function handleDelete() {
  if (
    confirm(
      `确定要删除这张工单【${props.order.customerDisplayName} - ${props.order.subcategoryName}】吗？`
    )
  ) {
    prototypeState.deleteWorkOrder(props.order.orderId)
    emit('back')
  }
}
</script>
