...
function onQtyChange() {
  ...
  if (!isNaN(qty) && qty > 0) {
    prototypeState.updateWorkOrder(props.order.orderId, { quantity: qty })
  } else {
...
function onPriceChange() {
  if (editPriceStr.value.trim() === '') {
    prototypeState.updateWorkOrder(props.order.orderId, { unitPriceCents: null })
  } else {
    const p = parseFloat(editPriceStr.value)
    if (!isNaN(p)) {
      prototypeState.updateWorkOrder(props.order.orderId, { unitPriceCents: Math.round(p * 100) })
    } else {
