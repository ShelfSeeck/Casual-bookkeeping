...
const activeOrder = ref<MockWorkOrder | null>(null)

function openOrderDetail(order: MockWorkOrder) {
      ...
      <header class="cb-ledger-header">
        <div class="cb-header-title-bar">
          ...
          <span class="cb-header-badge">
            {{ prototypeState.filteredOrders.value.length }} 笔记录
          </span>
        </div>
        <LedgerFilterBar />
      </header>
      ...
      <div class="cb-summary-compact-strip cb-tabular-nums" role="status" aria-label="数据汇总">
        <div class="cb-sum-left">
          <span class="cb-sum-item">
            共 <strong class="cb-sum-strong">{{ prototypeState.ledgerSummary.value.totalCount }}</strong> 笔
          </span>
          <span class="cb-sum-dot">·</span>
          <span class="cb-sum-item">
            <strong class="cb-sum-strong">{{ prototypeState.ledgerSummary.value.totalPieces.toLocaleString() }}</strong> 件
          </span>
          <span v-if="prototypeState.ledgerSummary.value.unpricedCount > 0" class="cb-unpriced-flag">
            ({{ prototypeState.ledgerSummary.value.unpricedCount }}单待定)
          </span>
        </div>
        <div class="cb-sum-right">
          <span class="cb-sum-money-label">合计:</span>
          <span class="cb-sum-money-val">¥{{ prototypeState.ledgerSummary.value.totalAmountYuan }}</span>
        </div>
      </div>

      <!-- 工单卡片列表 -->
      <main class="cb-ledger-body">
        <div v-if="prototypeState.filteredOrders.value.length === 0" class="cb-empty-ledger">
          ...
          <button
            ...
            @click="
              prototypeState.ledgerFilters.customerId = null;
              prototypeState.ledgerFilters.categoryName = null;
              prototypeState.ledgerFilters.status = 'all';
              prototypeState.ledgerFilters.searchKeyword = '';
              prototypeState.ledgerFilters.datePreset = 'today';
            "
          >
            重置筛选
          </button>
        ...
        <div v-else class="cb-order-list">
          <WorkOrderCard
            v-for="order in prototypeState.filteredOrders.value"
            :key="order.orderId"
            :order="order"
